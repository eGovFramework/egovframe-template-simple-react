...
const EgovWeeklyList = (props) => {
  ...
  const getWeekOfMonth = (date) => {
    ...
    const weeksOrder = [0, 1, 2, 3, 4, 5]
    const returnVal = parseInt(weeksOrder[0 | (adjustedDate / 7)])
  ...
  const drawList = () => {
    ...
    for (let dayIdx = 0; dayIdx < 7; dayIdx++) {
      const scheduleDate = new Date(searchCondition.year, searchCondition.month, searchCondition.date + dayIdx)
      const scheduleDateStr = `${scheduleDate.getFullYear()}년 ${
        scheduleDate.getMonth() + 1
      }월 ${scheduleDate.getDate()}일 ${dayNames[scheduleDate.getDay()]}`
      keyPropertyCnt++
      mutListTag.push(
  ...
  const getTimeForm = (str) => {
    ...
    const starminute = str.substring(10, 12)
