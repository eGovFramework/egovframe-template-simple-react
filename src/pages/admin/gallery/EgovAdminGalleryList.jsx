    ...
    const retrieveList = (searchCondition) => {
        ...
        EgovNet.requestFetch(retrieveListURL,
            ...
            (resp) => {
                ...
                let resultCnt = parseInt(resp.result.resultCnt);
                let currentPageNo = resp.result.paginationInfo.currentPageNo;
                let pageSize = resp.result.paginationInfo.pageSize;

                // 리스트 항목 구성
                const mutListTag = resp.result.resultList.map(function (item, index) {
                    const listIdx = itemIdxByPage(resultCnt , currentPageNo, pageSize, index);

                    return mutListTag.push(
                        <Link to={{
                ...
                if (!mutListTag.length) mutListTag.push(<p className="no_data">검색된 결과가 없습니다.</p>); // 게시판 목록 초기값
