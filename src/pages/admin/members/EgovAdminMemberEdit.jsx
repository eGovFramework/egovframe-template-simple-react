...
function EgovAdminMemberEdit(props) {
	...
	const checkIdDplct = () => {
		return new Promise((resolve, reject) => {
	        ...
	        EgovNet.requestFetch(checkIdURL,
	            reqOptions,
	            function (resp) {
    ...
    const deleteMember = (uniqId) => {
        ...
        EgovNet.requestFetch(deleteMemberURL,
            requestOptions,
            (resp) => {
                ...
                <div className="layout">
                    ...
                    <div className="contents BOARD_CREATE_REG" id="contents">
                        ...
                        <div className="board_view2">
                            <dl>
                                <dt><label htmlFor="mberId">회원ID</label><span className="req">필수</span></dt>
                                <dd>
                                	{/* 등록 일때 변경 가능 */}
                                    {modeInfo.mode === CODE.MODE_CREATE &&
                                    <>
                                    <input className="f_input2 w_full" type="text" name="mberId" title="" id="mberId" placeholder=""
                                        defaultValue={memberDetail.mberId}
                                        onChange={e => setMemberDetail({ ...memberDetail, mberId: e.target.value })}
										ref={el => (checkRef.current[0] = el)}
										required
                                    />
                                    <button className="btn btn_skyblue_h46" onClick={() => {
                                        checkIdDplct();
                                    }}>{memberDetail.checkIdResult}</button>
                                    </>
                                   }
                                   {/* 수정/조회 일때 변경 불가 */}
                                   {modeInfo.mode === CODE.MODE_MODIFY &&
                            ...
                            <dl>
                                <dt><label htmlFor="password">회원암호</label><span className="req">필수</span></dt>
                                <dd>
                                	{/* 등록 일때 변경 가능 */}
                                    {modeInfo.mode === CODE.MODE_CREATE &&
                                    <input className="f_input2 w_full" type="password" name="password" title="" id="password" placeholder=""
                                   ...
                                   {/* 수정/조회 일때 */}
                                   {modeInfo.mode === CODE.MODE_MODIFY &&
                                   <input className="f_input2 w_full" type="password" name="password" title="" id="password" placeholder="빈값이면 기존 암호가 변경되지 않고 그대로 유지됩니다."
                                        defaultValue=""
                                        onChange={e => setMemberDetail({ ...memberDetail, password: e.target.value })}
                                        ref={el => (checkRef.current[1] = el)}
                                    />
                            ...
                            <dl>
                                <dt><label htmlFor="bbsNm">회원명</label><span className="req">필수</span></dt>
                                <dd>
                                    <input className="f_input2 w_full" type="text" name="mberNm" title="" id="mberNm" placeholder=""
                            ...
                            <dl>
                                <dt>회원상태<span className="req">필수</span></dt>
                                <dd>
                            ...
                            <div className="board_btn_area">
                                ...
                                <div className="right_col btn1">
                                    <Link to={URL.ADMIN_MEMBERS} className="btn btn_blue_h46 w_100">목록</Link>
                                </div>
                            </div>
                            {/* <!--// 버튼영역 --> */}
                        </div>
